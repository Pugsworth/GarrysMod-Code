...
function PANEL:FirstPersonControls()
	...
	if ( self.MouseKey == MOUSE_LEFT ) then
		...
		local vOrigin = mins + ((maxs - mins) / 2)

		-- self.aLookAngle = (vOrigin - self.vCamPos):Angle() + Angle(y * 4, x * 4, 0)
		self.aLookAngle = self.aLookAngle + Angle( y * 4, x * 4, 0 )
		
		self.vCamPos = -self.aLookAngle:Forward() * self.vCamPos:Length()
...
derma.DefineControl( "DAdjustableModelPanelEx", "A panel containing a model", PANEL, "DModelPanelEx" )
