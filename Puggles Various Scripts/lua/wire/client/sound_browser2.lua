...
local function FormatLength(nduration, nsize)
	...
	return nduration, format("%01d:%02d.%03d", nm, ns, nms)
end
...
// Output the infos about the given sound. Used for the info text.
 local function GetInfoString(strfile, tabdata)
	...
	if (tabdata[1] == "property") then
		return format("Name: %s%s\n\rType: %s", strfile, strlength, tabdata[1])
	end

	local nsizeB, strsize = FormatSize(nsize, nduration)
	local strSize = "\n\rSize: "..(strsize and (strsize.." ("..nsizeB .." Bytes)") or "n/a")

	return format("Name: %s%s%s\n\rType: %s\n\rFormat: %s", strfile, strlength, strSize, (tabdata[1] or "file"), strformat)
end
	...
	if isstring(strSound) and strSound ~= "" then
		...
		//Print to Chat
			...
			if (len > max_char_chat_count) then
				...
				MenuItem:SetToolTip(format("The filepath (%i chars) is too long to print in chat. It should be shorter than %i chars!"))
			end
...
local function CreateSoundBrowser(path)
	...
	TabFileBrowser:SetRootName("All")
	TabFileBrowser:SetRootPath("sound")
	TabFileBrowser:SetPathID("GAME")
	TabFileBrowser:SetFileTypes({"*.mp3","*.wav", "*.ogg"})

	timer.Simple(0, function()
		TabFileBrowser.Root:SetIcon("games/16/all.png");
	end);
	...
	for i = 1, #games do
		local game = games[i];
		if game.mounted and game.owned then
			sorttbl[#sorttbl+1] = game.folder;
		end
	end
	...
	for i = 1, #sorttbl do
		local folder = sorttbl[i];
		local node = TabFileBrowser.Tree.RootNode:AddFolder(folder, "sound", folder, false);
		node:SetIcon(string.format("games/16/%s.png", folder));
	end

	local node = TabFileBrowser.Tree.RootNode:AddFolder("garrysmod", "sound", "garrysmod", false);
	node:SetIcon("games/16/garrysmod.png");
