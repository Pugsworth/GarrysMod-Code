...
AccessorFunc( PANEL, "m_strPathID",             "PathID" ) // "GAME", "DATA" etc.
AccessorFunc( PANEL, "m_tFilter",               "FileTypes" ) // "*.wav", *.mdl, {"*.vmt", "*.vtf"} etc.
...
local function BuildFileList(path, filter, pathid)
    local files = {}

    if (istable(filter)) then
        for k, v in ipairs(filter) do
            table.Add(files, file.Find(ConnectPathes(path, v), pathid or "GAME"))
        end
    else
        table.Add(files, file.Find(ConnectPathes(path, tostring(filter)), pathid or "GAME"))
    end
...
local function ShowFolder(self, path, pathid)
    if (!IsValid(self)) then return end

    self.m_strOpenPath = path
    path = ConnectPathes(self.m_strRootPath, path)

    self.Files = BuildFileList(path, self.m_tFilter, pathid)
    ...
    for i=1, self.m_nPageCount do
        ...
        button:SetToolTip(format("Page %i of %i", i, self.m_nPageCount))
...
function PANEL:Init()
    ...
    self.Tree.OnNodeSelected = function( parent, node )
        ...
        ShowFolder(self, path, node:GetPathID() or "GAME");
    end
...
function PANEL:UpdatePageToolTips()
    self.PageLeftButton:SetToolTip(format("Previous Page (%i of %i)", self.m_nPage - 1, self.m_nPageCount))
    self.PageLeftButton:SetToolTip(format("Next Page (%i of %i)", self.m_nPage + 1, self.m_nPageCount))

    self.PageLeftButton:SetToolTip(format("Last Page (%i of %i)", self.m_nPageCount, self.m_nPageCount))
    self.PageLastLeftButton:SetToolTip(format("First Page (1 of %i)", self.m_nPageCount))
end
...
function PANEL:Setup()
    ...
    self.Root = self.Tree.RootNode:AddFolder( self.m_strRootName, self.m_strRootPath, self.m_strPathID or "GAME", false)
...
function PANEL:SetPage(page)
    ...
    self.PageLoadingLabel:SetText(format("0 of %i files found.", FileCount))
    self.PageLoadingLabel:SizeToContents()
    self.PageLoadingLabel:Center()
    ...
    WireLib.Timedpairs(self.TimedpairsName, filepage, self.m_nListSpeed, function(id, name, self)
        ...
        self.PageLoadingLabel:SetText(format("%i of %i files found.", id, FileCount))
        self.PageLoadingLabel:SizeToContents()
        self.PageLoadingLabel:Center()
